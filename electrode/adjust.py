...
class Adjust():
	...
	def __init__(self, system):    # must input a System() instance.  wwc
		self.system = system
		volts = {}
		for ele in system:
			volts[ele.name] = []#[ele.dc]    # initial .dc
	...
	def set_move_init(self, E_config=None, xm=None, axis_order=None):
		...
		# 2. Deploy dc change/unchange configuration. E_config=[[ele, ele...],[ele, ele,..]...], [] encloses 
		# the electrodes with the same voltage changes.

		# minimum xm
		if xm == None:		# system.minimum() still has some problem, if it appears, you can manually input xm.
			xi = (0.,0.,0.)
		...
		# print("Eigenvectors:\n",evector)
		if axis_order == None:
			axis_order = evector[0].argsort()[::-1]    # sort according to the x coordinate of eigenvectors. [::-1] in acsending order
		evalue, evector = evalue[axis_order], evector[:,axis_order].T
		self.curves, self.basis = [evalue], evector
		ax_check = [(ax, ev) for ax, ev in zip("axial radial1 radial2".split(), evector)]	# print out axis to check.
			# print(ax+"axis:\t",ev)
	...
	def set_volts(self, config, change_rf=False, config_rf=None, save=False):
		# Set voltages for system. Dict config = {dc_name:dc_volt}, config_rf = {rf_name:rf_volt}. RF electrodes could also have DC bais.
		# save argument is for this class.
	...
	def xm_freq_change(self,deltaV):
		...
		xm_new = self.system.minimum(xm_old+0.01)    # or (0.,0.,0.). add +0.01 to aviod minimum precision loss
	...
	def next_step(self,deltaV=None,accept=True):
		...
		# Pay attention, if you have used propose_move() or xm_freq_change(), the deltaV have been applied to system.
		...
		if deltaV == None:
			try:
				...
				warnings.warn("Propose a volt_change() first.")    # cast a UserWarning
		...
		if accept:
			...
			self.set_volts(self.Volts[-1])		# reset voltages back to erase the change due to xm_freq_change()
			# print("Stay")
